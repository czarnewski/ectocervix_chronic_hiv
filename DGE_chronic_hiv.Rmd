...
entrez <- counts_all[, c("entrez","symbol")]
counts_all <- rowsum( counts_all[,-c(1:3) ] , counts_all$symbol )
...
# top <- read_xlsx(paste0(result_dir,"DGE_HIV_",run,".xlsx"))
```
...
```{r enrichment-analysis, include=FALSE, eval=FALSE}
# not used due to lack of background list functionality
...
### Save gene lists and background for use in enrichr browser

```{r gene-lists}

# Get gene lists with up and down regulated genes separately 
get_reg.fun <- function(FC_val, p_col, p_val){
  t <- top %>%
  mutate('reg' = ifelse(logFC >= FC_val & {{p_col}} <= p_val ,"Up", 
                        ifelse(logFC <= -FC_val & {{p_col}}  <= p_val, "Down", 'NotSig'))) %>%
...
down <- top %>% filter(symbol %in% names(h_HIV[h_HIV=="Down"])) %>% select(down="symbol")
up <-  top %>% filter(symbol %in% names(h_HIV[h_HIV=="Up"])) %>% select(up="symbol")
background <- select(counts_all, background="symbol")

# combine gene lists in one table
as_tibble(background) %>%
  rownames_to_column(.) %>%
  left_join(.,  rownames_to_column(up) ) %>%
  left_join(.,  rownames_to_column(down) ) %>%
  select(-rowname) %>%
  write.xlsx( ., file = paste0(result_dir,"Enrichr_gene_lists.xlsx"))

```


```{r enrichr-API}
#### ACCES ENRICHR API THROUGH R ####
remotes::install_github("MaayanLab/backr/backr")
library(backr)
library(jsonlite)
library(httr) 
...
# Use all 15 435 genes as background
bg = background$background
up <- up$up
down <- down$down
db <- c("GO_Biological_Process_2023","KEGG_2021_Human","ARCHS4_TFs_Coexp","MSigDB_Hallmark_2020") # "TRRUST_Transcription_Factors_2019",
# the TRRUST database did not work so we went with the manual way instead, see instructions bellow
# NB! the column names are different from the ones produced by manually downloading the results from the website

result <- tibble(genes=list(up, down), db = list(db, db)) %>% 
  unnest(db) %>%
  pmap(., ~backr::enrich(..2, gene_array = ..1, background = bg)) %>%
  set_names(., unlist(map(c("Up", "Down"), ~paste0(str_extract(db, "^[^_]+"), "_", .x))))

# Save to excel file
write.xlsx( result, file = paste0("./results/enricr_browser/","FEA_HIV_",run,".xlsx"))


#### MANUAL ALTERNATIVE ####

# The gene lists was copy pasted into the enrichr web interface on 21th of October 2024
# up and down regulated genes was queried separately
# https://maayanlab.cloud/Enrichr/enrich
# the databases used was:
...
# Combine all tables downloaded from the enrichr website into one excel workbook:
folders <- c("Up", "Down") %>% set_names()
pattern <- paste0(c("GO","KEGG", "MSigDB", "ARCHS4", "TRRUST"), collapse = "|")

file_list <- folders %>%
  # Get list of all .txt files in both folders
  map(., ~list.files(path = paste0(getwd(),"/results/enricr_browser/", .x, "/"),  pattern = "\\.txt$", full.names = TRUE)) %>%
  enframe() %>%
  mutate( names = pmap(., ~paste0(str_extract_all(..2, pattern), "_", ..1))) %>%
  # Create a named list of file names
  mutate( value = pmap(., ~set_names(..2, ..3))) %>% 
  .$value %>%
  flatten_chr(.) %>%
...
# Save to excel file
write.xlsx( file_list , file = paste0("./results/enricr_browser/","FEA_HIV_",run,".xlsx"))
```
...
n <- 10
comparison <- c("HIV")
enrichment_db <- c("GO","KEGG","ARCHS4","TRRUST","MSigDB")
path <- paste0("./results/enricr_browser/","FEA_HIV_",run,".xlsx") # browser results
result_dir <- "./results/enricr_browser/"
...
for( i in enrichment_db ){
  ...
  f_DOWN <- read_xlsx(path, sheet = paste0(i,"_Down")) %>% filter(`Adjusted P-value` <= 0.05)
  f_UP <- read_xlsx(path, sheet = paste0(i,"_Up")) %>% filter(`Adjusted P-value` <= 0.05) # Adjusted.P.value
  
  if (is.data.frame(f_DOWN) && nrow(f_DOWN)>0){
    tempD_e <- f_DOWN
    if (i == "GO")
      {tempD_e <- separate_wider_regex(tempD_e, cols=Term, pattern=c(Term=".+(?=\\s\\()", "\\s\\(", GOid="GO\\:\\d+", "\\)"), too_few = "align_start")}
  ...
  if (is.data.frame(f_UP) && nrow(f_UP)>0){
    tempU_e <- f_UP
    if (i == "GO")
      {tempU_e <- separate_wider_regex(tempU_e, cols=Term, pattern=c(Term=".+(?=\\s\\()", "\\s\\(", GOid="GO\\:\\d+", "\\)"), too_few = "align_start")}
